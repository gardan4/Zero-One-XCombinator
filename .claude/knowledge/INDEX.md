...
- 2026-05-30 — **Track eval/docs:** `docs/track-industrial-sources.md` (missing `eval_metrics.py`/kickoff CSVs/unpublished rubrics; HF **`XCombinator`** + W&B **`XCombinator/XCombinator`**). `zo-track` requires `--version`; writes `metrics_report.json/md`; per-cut `_frac60/_frac80` metrics; predictors incl. `likelihood-ngram`/`classifier`. — (eval, track-industrial-ai)
