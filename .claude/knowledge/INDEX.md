...
- (resolved) From-scratch token model dropped → **fine-tune an open base (Qwen2.5)** via SFT/GRPO; configs in `packages/training/configs/` (`sft_fab*`, `grpo_fab*`, `leonardo_sft_fab*`) → [track-industrial-ai.md](track-industrial-ai.md) + [training.md](training.md)
...
- 2026-05-30 — **Base-model next-step baseline (`base`/`base-hf` predictors):** an un-fine-tuned LLM given ALL relevant context (family + legal step vocab + recent-step descriptions + optional reference recipe + process-so-far) via `datagen.*_context_example` + `predict_llm` `style="base"`; default `Qwen/Qwen2.5-7B-Instruct` (`ZO_BASE_LLM_MODEL`), `ZO_BASE_PROMPT_REFERENCE=0` for the no-lookup variant. Wired into the `/inference` dropdown (tags `predictor:base[-hf]` → `/compare`). `hub_inference` now loads fp16/bf16 not fp32. Verified on M5 Pro/MPS with 0.5B (all preds in-vocab, gold in top-3 at frac60). 108 tests green. **Use 0.5B/1.5B locally; 7B is slow on a laptop.** — (eval, track-industrial-ai)
