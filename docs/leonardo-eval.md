...
| `ZO_EXPERIMENTS_DIR` | `$SCRATCH/zo-experiments` (local scratch; W&B is durable store) |
| `WANDB_API_KEY` | Training + eval metrics on W&B (compute: `WANDB_MODE=offline`, sync on login) |
...
Pre-stage GPU deps **once** on the login node (compute nodes have no internet). The pip path is
the smallest one for graders; `uv`/`just` are optional teammate shortcuts.

```bash
# pip + zo_cluster (no uv required on the laptop):
...
# just judge-stage
# just judge-eval --local --model ... --train-run ... --tags real-run,reportable,...
# just cluster-watch
```

Results land in `$ZO_EXPERIMENTS_DIR/<run_id>/results/` (local scratch). Metrics + CSVs also go to
**W&B** when `WANDB_API_KEY` is set. See [eval-and-artifacts.md](eval-and-artifacts.md).
